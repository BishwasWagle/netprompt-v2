...
4. **Magic-string vocabularies** (outcomes / switch-status / metrics) duplicated
   across 6 files.
Plus MEDIUM/LOW items (margin-formula dup, in-memory `last_good`, docstring-only
protocols, per-statement KG sessions…) and an **appendix of 3 *cleared* claims**
...
- **This file is generated from the docs it summarizes — keep it in sync.** As its closing note states, the digest is derived from the six review documents it covers, so if those change the relevant section here must be updated. The master facts table and the current-implementation-status table give you the verified numbers and commit hashes at a glance without re-reading the source docs.
