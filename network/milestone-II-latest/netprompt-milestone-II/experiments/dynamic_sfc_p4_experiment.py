...
def apply_sfc_queue_policy(net, sfc):
    ...
    if sfc == "LowLatencyVideoSFC":
        ...
        # Energy saving: allow reduced-rate essential traffic from d4/d5/d6.
        for host in [d4, d5, d6]:
            host.cmd(
                f"tc qdisc replace dev {host.name}-eth0 root tbf "
                "rate 5mbit burst 32kbit latency 400ms"
            )
            applied.append(f"{host.name}-eth0: tbf rate 5mbit burst 32kbit latency 400ms")
